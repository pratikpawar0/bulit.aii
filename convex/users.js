...
export const getByUsername = query({
  ...
  handler: async (ctx, args) => {
    // For now, we'll search by name since username isn't in the schema
    // You may need to add a username field to your schema
    const user = await ctx.db
      .query("users")
...
export const updateUsername = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // For now, we'll update the name field
    // You may want to add a separate username field to your schema
    await ctx.db.patch(user._id, {
      name: args.username,
      updatedAt: Date.now(),
    });
